...
En utilisant n_bits = 10, 20, 30, 40 bits

Pour tester les signaux 0, 2, 4, 6, 8, 10

On peut changer les parametres pour tester aussi les signaux 1, 3, 5, 7, 9
...
class SignalCodingEnv:
    def __init__(self, signal_segments, num, n_action, nx_bit):
    ...
    def step(self, action):
        ...
        #state = self.state
        self.snrs[index, action] = model_snr(signal, index_s, action, self.nx_bit, 0, self.num_phase)
...
env = SignalCodingEnv(x, nb_w, n_action, 10)
...
'''
action_estimee10 = np.zeros(nb_w*6) 
action_estimee20 = np.zeros(nb_w*6) 
action_estimee30 = np.zeros(nb_w*6) 
action_estimee40 = np.zeros(nb_w*6) 
'''

snr_estimee10 = np.zeros(nb_w*6) 
snr_estimee20 = np.zeros(nb_w*6) 
snr_estimee30 = np.zeros(nb_w*6) 
snr_estimee40 = np.zeros(nb_w*6) 

action_best2 = np.zeros(nb_w*6) 
snr_best2 = np.zeros(nb_w*6) 

nx_choix = np.zeros(nb_w*6)
nx_best = np.zeros(nb_w*6)
...
for i in range(nb_w*6):
    ...
    #action_estimee10[i] = action1
    snr1 = model_snr(x3[id_s], signal_index, action1, 10, 0, num_phase)
    snr_estimee10[i] = snr1
    
    q_values2 = dqn2.predict(state_n)
    action2 = np.argmax(q_values2)
    #action_estimee20[i] = action2
    snr2 = model_snr(x3[id_s], signal_index, action2, 20, 0, num_phase)
    snr_estimee20[i] = snr2
    
    q_values3 = dqn3.predict(state_n)
    action3 = np.argmax(q_values3)
    #action_estimee30[i] = action3
    snr3 = model_snr(x3[id_s], signal_index, action3, 30, 0, num_phase)
    snr_estimee30[i] = snr3
    
    q_values4 = dqn4.predict(state_n)
    action4 = np.argmax(q_values4)
    #action_estimee40[i] = action4
    snr4 = model_snr(x3[id_s], signal_index, action4, 40, 0, num_phase)
    snr_estimee40[i] = snr4
    
    # 找到最大 SNR 和对应的 action
    snr_values = [snr1, snr2, snr3, snr4]
    actions = [action1, action2, action3, action4]
    max_snr_index = np.argmax(snr_values)
    
    snr_estimee[i] = snr_values[max_snr_index]
    action_estimee[i] = actions[max_snr_index]
    nx_choix[i] = max_snr_index
    ...
    for k in range(4):
        for j in range(n_action):
            snr = model_snr(x3[id_s], signal_index, j, (k+1)*10, 0, num_phase)
            if snr > snr_b:
                snr_b = snr
                action_b = j
                nx_b = k
...
# 计算混淆矩阵
cm_nx = confusion_matrix(nx_best, nx_choix, labels=[0, 1, 2, 3])

# 打印混淆矩阵
print("Confusion Matrix nx:")
print(cm_nx)
...
for i in range(6):
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(time_stamps, nx_choix[i*nb_w:(i+1)*nb_w], c='blue', marker='o')
    plt.title(f'Nx Selection For Signal {2*i}')
    plt.xlabel('Time (s)')
    plt.ylabel('Selected Model')
    plt.xticks(np.linspace(0, 2, 11))
    plt.yticks(range(5))  # 假设模型编号从 0 开始
    plt.grid(True)
    plt.show()
    
    # 绘制散点图
    plt.figure(figsize=(10, 6))
    plt.scatter(time_stamps, nx_best[i*nb_w:(i+1)*nb_w], c='blue', marker='o')
    plt.title(f'Best Nx Selection For Signal {2*i}')
    plt.xlabel('Time (s)')
    plt.ylabel('Selected Model')
    plt.xticks(np.linspace(0, 2, 11))  # 生成0到2秒的11个等间隔刻度
    plt.yticks(range(5))  # 假设模型编号从 0 开始
    ...
    # 创建一个新的图形
    plt.figure(figsize=(10, 6))
    plt.scatter(time_stamps, nx_choix[i*nb_w:(i+1)*nb_w], c='blue', marker='o', label='Estimated Model')
    plt.scatter(time_stamps, nx_best[i*nb_w:(i+1)*nb_w], c='red', marker='o', label='Best Model')
    plt.title(f'Nx Selection Comparasion For Signal {2*i}')
    plt.xlabel('Time (s)')
    plt.ylabel('Selected Model')
    plt.xticks(np.linspace(0, 2, 11))  # 生成0到2秒的11个等间隔刻度
    plt.yticks(range(5))  # 假设模型编号从 0 开始
    ...
    # 创建一个新的图形
    plt.figure(figsize=(10, 6))    
    plt.plot(time_stamps, snr_estimee10[i*nb_w:(i+1)*nb_w], 'b--', marker='o', label='SNR for 10 bits')
    plt.plot(time_stamps, snr_estimee20[i*nb_w:(i+1)*nb_w], 'g--', marker='o', label='SNR for 20 bits')
    plt.plot(time_stamps, snr_estimee30[i*nb_w:(i+1)*nb_w], 'c--', marker='o', label='SNR for 30 bits')
    plt.plot(time_stamps, snr_estimee40[i*nb_w:(i+1)*nb_w], 'm--', marker='o', label='SNR for 40 bits')
    plt.plot(time_stamps, snr_estimee[i*nb_w:(i+1)*nb_w], 'r-', marker='o', label='SNR Estimee')    

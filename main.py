...
for factor in [2,4,10]:
    ...
    id_previous_model=2 # model used in the previous window
...
nb_w=100 # number of widow per signals
...
R_unused=np.zeros((nb_phase,nb_w)) # rate not used
...
for phase in range(nb_phase):
    for w in range(nb_w): 
        ...
        dict_MMC_enc[phase].MMC_enc(x_test,x_previous,n_max)
...
for phase in range(nb_phase):
    ...
    """
    plt.figure(figsize=(width,4), dpi=100)
    plt.bar([t[k] for k in range(0,nb_w*N,N)],100*R_h[phase]/n_max, width = largeur_barre,color='g')
    plt.bar([t[k] for k in range(0,nb_w*N,N)],100*R_m[phase]/n_max, width = largeur_barre, bottom =100*R_h[phase]/n_max,color='r')
    plt.bar([t[k] for k in range(0,nb_w*N,N)],100*R_r[phase]/n_max, width = largeur_barre, bottom =100*(R_h[phase]+R_m[phase])/n_max,color='b')
    plt.bar([t[k] for k in range(0,nb_w*N,N)],100*(n_max-R_h[phase]-R_m[phase]-R_r[phase])/n_max, width = largeur_barre, bottom =100*(R_h[phase]+R_m[phase]+R_r[phase])/n_max,color='y')
    plt.xlabel('t (s)')
    plt.ylabel('Rate (%)')

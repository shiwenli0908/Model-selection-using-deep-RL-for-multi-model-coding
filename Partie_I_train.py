...
Les sorties sont choix des modeles

Entrainer le reseau en utilisant les signaux 1, 3, 5, 7, 9
...
# 保存训练后的模型
save_model(dqn.model, "dqn_model10bits.h5")

'''
# 加载保存的模型
trained_model = load_trained_model("dqn_model2.h5")
...
for i in range(nb_w*5):
    ...
    for j in range(n_action):
        snr = model_snr(x[id_s], signal_index, j, num_phase)
        if snr >= snr_b:
            snr_b = snr
            action_b = j
    action_best[i] = action_b
    snr_best[i] = snr_b
...
for i in range(nb_w*5):
    ...
    num_phase = i // 600
    signal_index = i % 100
    model = action_choix[i]
    snr_choix[i] = model_snr(x[id_s], signal_index, model, num_phase)

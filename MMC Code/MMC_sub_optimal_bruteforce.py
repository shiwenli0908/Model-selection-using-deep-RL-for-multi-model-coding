...
class Encode_one_window(Model_Encoder,Residual_Encoder):
    ...
    def MMC_enc(self,x,x_previous,n_max):
        ...
        SNR_tot=-np.infty
        SNR_tot_save=np.zeros(n_model_max)
            ...
            if SNR_model>=0:
                ...
                for id_residual_test in self.best_Residual_used :   
                    ...
                    if SNR_residual_test > SNR_residual:
                        ...
                        SNR_tot_save[n_model]=SNR_model+SNR_residual
        ...
        positions=[]
        values=[]
        for index, value in enumerate(SNR_tot_save):
            if value > 0:
                positions.append(index)
                values.append(value)
        plt.plot(positions,values,'-*',lw=1)
        plt.xlabel('n_x+n_nx+n_kx+n_kr')
        plt.ylabel('SNR_tot')
        plt.title("evolution of n tot")
        ...
        print("nx",self.nx_enc)
        print("nr",self.nr_enc)
        print("n nx",self.n_nx_enc)
        print("m",len(code_m))
        print("l",len(code_l))
        print("n_kx_enc",self.n_kx_enc)
        print("n_kr_enc",self.n_kr_enc)
        """
        return code

...
Tester en utilisant les signaux 0, 2, 4, 6, 8, 10
"""
...
trained_model = load_trained_model("dqn_model_Algo2.h5")
...
# 保存训练后的模型
save_model(dqn.model, "dqn_model_Algo.h5")
'''
...
action_choix = np.zeros(nb_w*6)
algo_choix = np.zeros(nb_w*6)
...
for i in range(nb_w*6):
    ...
    action_choix[i] = action1
    algo_choix[i] = action1 // 4


snr_choix = np.zeros(nb_w*6)
for i in range(nb_w*6):
    id_s = (i % 600) // 100
    num_phase = i // 600
    signal_index = i % 100
    ...
    snr_choix[i] = model_snr(x3[id_s], signal_index, model_i, (nx_i+1)*10, algo_i, num_phase)
...
action_best = np.zeros(nb_w*6)
algo_best = np.zeros(nb_w*6)
snr_best = np.zeros(nb_w*6)
for i in range(nb_w*6):
    id_s = (i % 600) // 100
    num_phase = i // 600
    signal_index = i % 100
    ...
    for j in range(2):
        for k in range(4):
            ...
            snr = model_snr(x3[id_s], signal_index, index_m, (k+1)*10, j, num_phase)
            if snr >= snr_b:
                snr_b = snr
                algo_b = j
                nx_b = k
